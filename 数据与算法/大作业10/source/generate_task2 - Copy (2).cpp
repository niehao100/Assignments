...
int main(int argc,char* argv[])
{
    int x;
    double x_double;
    sscanf(argv[10],"%lf",&x_double);
    x=x_double*100;
    if(x<0||x>100)
    {
        fprintf(stderr,"Bad x=%d.\n",x);
        return -1;
    }
    int
    for(int i=0;i<9;++i)
    {
        Status status0=Put(status0,i); //Step1, x on position i
        for(int j=NextEmptyPosition(status0,0);j>=0;j=NextEmptyPosition(status0,j+1))
        {
            Status status1=Put(status0,j);  //Step2, o on position j
