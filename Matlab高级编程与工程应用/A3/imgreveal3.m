function [img, msg] = imgreveal3(dcbin,acbin,m,n,DCTAB,ACTAB,QTAB)
    ...
    for k = 1:32
        msglength(k) = (coefficients(find(coefficients(:,k),1,'last'),k)==1);
    end
    msglength = sum(2.^[31:-1:0].*msglength);
    msg = false(1,msglength);
    for k = 1:msglength
        msg(k) = (coefficients(find(coefficients(:,k+32),1,'last'),k+32)==1);
    end
end
